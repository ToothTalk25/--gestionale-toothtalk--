-- =====================================================================
-- 0058_blocco_riconoscimento_su_segnalazione.sql
--
-- Due correzioni:
--
-- 1) Bug di regressione in sigilla_pacchetto: la 0052 (liberatoria_solo_otp)
--    ha ridefinito la funzione ripartendo dalla 0041 invece che dalla 0050,
--    perdendo per errore il controllo Fase D (riconoscimento automatico) E
--    l'assegnazione di v_quando_s, che di conseguenza finiva sempre NULL nel
--    manifesto sigillato ('sigillato_at' mai valorizzato dalla 0052 in poi).
--    Qui si ripristinano entrambi, sopra al testo già corretto della 0052
--    (controllo liberatoria via OTP).
--
-- 2) Il controllo Fase D era applicato solo al sigillo (azione admin), non
--    alla segnalazione di completamento (azione del gruppo): un gruppo
--    poteva quindi segnalare completo un pacchetto con una persona esterna
--    non dichiarata, e il blocco arrivava solo alla revisione admin. Si
--    sposta lo stesso controllo anche su segnala_completato, così il gruppo
--    lo vede subito, al momento in cui prova a mandarlo in revisione.
-- =====================================================================
...
begin
  ...
  select esito into v_esito_riconoscimento
  from public.verifiche_riconoscimento
  where pacchetto_id = p_pacchetto
  order by creato_at desc
  limit 1;

  if v_esito_riconoscimento = 'persona_non_riconosciuta' and not v_coinvolge_terzi then
    raise exception 'Il controllo automatico ha rilevato nel video una persona che non corrisponde a nessun membro del gruppo. Se è presente una persona esterna, spunta "Il video mostra una persona esterna" e invita quella persona a firmare la liberatoria: senza liberatoria firmata non è possibile procedere.';
  end if;
