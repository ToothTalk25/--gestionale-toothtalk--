-- =====================================================================
-- 0059_riconoscimento_anche_copertina.sql
--
-- Il controllo automatico (Fase D) girava solo sul video: se si sostituiva
-- il video con uno "pulito" ma si lasciava la vecchia copertina con una
-- persona esterna non dichiarata, nulla lo rilevava. Si estende il
-- controllo anche alla copertina, e si tiene traccia di QUALE elemento
-- (video o copertina) è stato verificato, così l'ultimo esito di uno non
-- nasconde più un vecchio esito negativo dell'altro.
-- =====================================================================

alter table public.verifiche_riconoscimento
  add column if not exists ruolo text not null default 'video'
  check (ruolo in ('video', 'copertina'));

alter table public.verifiche_riconoscimento alter column ruolo drop default;

create index if not exists idx_verifiche_riconoscimento_pacchetto_ruolo
  on public.verifiche_riconoscimento(pacchetto_id, ruolo, creato_at desc);
