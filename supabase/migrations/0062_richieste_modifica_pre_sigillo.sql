-- =====================================================================
-- 0062_richieste_modifica_pre_sigillo.sql
--
-- Le richieste di modifica cambiano ruolo: non sono più correzioni su un
-- video già sigillato (il pacchetto sigillato resta immutabile per sempre,
-- per quello serve "Annulla pacchetto", che non cancella nulla e lascia il
-- sigillo originale a registro). Ora sono lo strumento con cui chi ha
-- accesso globale, revisionando un pacchetto "pronto", segnala cosa va
-- corretto PRIMA di sigillare:
--
--   - non si può aprire una richiesta su un pacchetto già sigillato/spedito
--     (va usato "Annulla pacchetto" per quel caso);
...
--   - non si può sigillare (né segnalare completato) finché resta una
--     richiesta di modifica ancora aperta sul progetto.
-- =====================================================================

create or replace function public.apri_richiesta_modifica(
  p_task uuid,
  p_pacchetto uuid,
  p_ambito public.ambito_richiesta,
  p_testo text
) returns uuid
...
begin
  ...
  if coalesce(btrim(p_testo), '') = '' then
    raise exception 'Scrivi cosa va corretto';
  end if;

  if p_pacchetto is not null then
    select * into v_p from public.pacchetti_video where id = p_pacchetto for update;
    if found and v_p.stato in ('sigillato', 'pec_inviata', 'pec_confermata', 'pec_errore') then
      raise exception 'Il pacchetto è già sigillato: usa "Annulla pacchetto" per farne comporre uno nuovo, da qui non è più correggibile';
    end if;
  end if;

  insert into public.richieste_modifica (task_id, pacchetto_id, ambito, testo, creata_da)
  values (p_task, p_pacchetto, p_ambito, btrim(p_testo), auth.uid())
  returning id into v_id;

  if v_p.stato = 'pronto' then
    perform public.rimanda_in_composizione(p_pacchetto);
  end if;

  return v_id;
end $$;

grant execute on function public.apri_richiesta_modifica(uuid, uuid, public.ambito_richiesta, text) to authenticated;
...
begin
  ...
  if exists (
    ...
    where task_id = v_p.task_id and stato = 'aperta'
  ) then
    raise exception 'Ci sono richieste di modifica ancora aperte su questo progetto: risolvile prima di segnalare il pacchetto come completato';
  end if;
...
begin
  ...
  if exists (
    ...
    raise exception 'Ci sono richieste di modifica ancora aperte su questo progetto: risolvile prima di sigillare';
  end if;
